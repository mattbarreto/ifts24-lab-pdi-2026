...
```powershell
$env:UV_PROJECT_ENVIRONMENT = ".venv_004"
uv venv .venv_004 --python 3.12
uv sync
```

> ⚠️ `UV_PROJECT_ENVIRONMENT` solo dura mientras esa terminal esté abierta. Si cerrás la terminal y abrís otra para correr `uv sync`, `uv run` o `uv lock` en esta carpeta, hay que volver a setear la variable antes (`$env:UV_PROJECT_ENVIRONMENT = ".venv_004"`) — si no, uv va a crear un `.venv` genérico nuevo en vez de usar `.venv_004`.

`uv sync` crea `.venv_004` con la versión de Python fijada en `.python-version` (3.12) e instala las versiones fijadas en `uv.lock`. El archivo `pyproject.toml` contiene las dependencias directas del laboratorio (numpy, opencv-python, pillow, matplotlib) más `ipykernel` para poder usar el entorno como kernel de Jupyter/VS Code.
...
```powershell
.venv_004\Scripts\python.exe -m ipykernel install --user --name pdi-004-cv1 --display-name "PDI 004 - Computer Vision 1"
```
...
```powershell
.venv_004\Scripts\Activate.ps1
```

En macOS o Linux:

```bash
source .venv_004/bin/activate
```

## Abrir los notebooks

La forma recomendada es VS Code (ver [instalación y selección del kernel](INSTALACION_VSCODE.md)): abrir esta carpeta en VS Code y seleccionar `.venv_004\Scripts\python.exe` (Windows) o `.venv_004/bin/python` (macOS/Linux) como intérprete/kernel.
