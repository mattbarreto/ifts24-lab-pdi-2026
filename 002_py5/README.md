...
`uv sync` crea `.venv_002` con la versión de Python fijada en `.python-version` (3.12) e instala las versiones fijadas en `uv.lock`. El archivo `pyproject.toml` contiene las dependencias directas del laboratorio (`py5`, `install-jdk`) más `ipykernel` para poder usar el entorno como kernel de Jupyter/VS Code.
