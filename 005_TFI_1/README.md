...
Trabajo Final Integrador 1: construcción de tres pipelines de mejora y restauración de imágenes (cámara oscura, medio gráfico color, medio gráfico blanco/negro). Ver la consigna completa y la rúbrica en `TFI_1_Consigna_y_Rubrica.md`. Esta carpeta puede usarse de forma independiente porque incluye su propia configuración de dependencias con `uv`.
...
```powershell
$env:UV_PROJECT_ENVIRONMENT = ".venv_005"
uv venv .venv_005 --python 3.12
uv sync
```

> ⚠️ `UV_PROJECT_ENVIRONMENT` solo dura mientras esa terminal esté abierta. Si cerrás la terminal y abrís otra para correr `uv sync`, `uv run` o `uv lock` en esta carpeta, hay que volver a setear la variable antes (`$env:UV_PROJECT_ENVIRONMENT = ".venv_005"`) — si no, uv va a crear un `.venv` genérico nuevo en vez de usar `.venv_005`.

`uv sync` crea `.venv_005` con la versión de Python fijada en `.python-version` (3.12) e instala las versiones fijadas en `uv.lock`. El archivo `pyproject.toml` contiene las dependencias directas del laboratorio (numpy, opencv-python, pandas, matplotlib) más `ipykernel` para poder usar el entorno como kernel de Jupyter/VS Code.
...
```powershell
.venv_005\Scripts\python.exe -m ipykernel install --user --name pdi-005-tfi1 --display-name "PDI 005 - TFI 1"
```

Esto registra el kernel en tu carpeta de Jupyter (con `--user`), así que va a aparecer en VS Code/Jupyter identificado como **"PDI 005 - TFI 1"**, sin mezclarse con los kernels de las demás carpetas.
...
```powershell
.venv_005\Scripts\Activate.ps1
```

En macOS o Linux:

```bash
source .venv_005/bin/activate
```

## Abrir el notebook

La forma recomendada es VS Code (ver [instalación y selección del kernel](INSTALACION_VSCODE.md)): abrir esta carpeta en VS Code y seleccionar `.venv_005\Scripts\python.exe` (Windows) o `.venv_005/bin/python` (macOS/Linux) como intérprete/kernel.
